...
**Loading rule / Regla de carga:** show this index immediately, but load only the one resource required by the current task; never load all resources by default. For architecture reviews read `references/elite-architecture-review.md`; for benchmarks, red-team or agent scoring read `references/agent-evaluation-lab.md`; for image, audio, video or long-running context read `references/multimodal-memory-design.md`; for comparisons with current models, benchmarks or frameworks read `references/public-patterns-2026.md`; for premium UI, CSS, bento, glass, container queries or scroll motion read `references/nexus-zero-ui-engine.md`.
## Problem / Problema
